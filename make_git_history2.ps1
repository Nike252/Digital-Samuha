...
Write-Host "Safely creating a new orphaned branch for the clean history..."
# Switch to a fresh orphan branch (no history)
git checkout --orphan new_history

# Clear the git tracking index without deleting any real files
git rm -rf --cached .
...
function New-HistoricalCommit {
    ...
    if ($WithFiles) {
        ...
        # Needs at least one commit to use --allow-empty? Actually, the very first commit can't be empty in git unless specified.
        # git commit --allow-empty works for initial commit too.
